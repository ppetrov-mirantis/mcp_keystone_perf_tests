...
for jmx_name in glob.glob(jmx_home + "*.jmx"):
    ...
    with open(percentiles_file, 'rb') as synth_report_csv_file:        
        ...
        statistic_records = commands.getstatusoutput("cat " + percentiles_file + \
                                                  " | grep -iE '^(50\.0|90\.0)'")[1].split("\n")
    ...
    with open(synthesis_file, 'rb') as synth_report_csv_file:
        for test_operation_name in reports[curr_test_plan_name].keys():
            operation_stats_record = commands.getstatusoutput("cat " + synthesis_file + \ 
                                                           " | grep -i '^" + test_operation_name + "'")[1].replace('"', r'\"')\
                                                                                                          .replace('(', r'\(')\
                                                                                                          .replace(')', r'\)')
...
test_run_name = "{0} {1} #{2}-{3}".format(product_version, test_suite_name, repo_snapshot_id,\
...
for test_report in reports.keys():
    ...
    for test_operation in test_operations.keys():
        ...
        for param_name, expected_value in testrail_expected_results.get(int(test_case_id)).items():
            ...
            testrail_all_additional_results.append({u'content':param_name,u'expected':expected_value,\
                                                    u'actual':actual,u'status_id':status_id})
        ...
        if (low_rps or many_errors or high_resp_time_median or high_90_percentile):
            test_case_global_status_id = 5
        
        #Sending results to TestRail
        testrun_comment = estimated_test_duration + cluster_config
        print testrail_client.send_post("add_result_for_case/" + str(test_run_id) + "/" \
                                          + test_case_id, {"status_id": test_case_global_status_id,\
                                                           "created_by": 89,\
